...
const ParkChairs = ({ updateBasketCount, loggedInUser }) => {
  ...
  return (
    <div className='px-12 py-6 m-11'>
      <h1 className='text-4xl font-medium text-center mb-8'>{t.parkChairs}</h1>
        ...
        {data.map((product) => {
          ...
          return (
            ...
            >
                ...
                <div>
                  <div
                    className={`absolute top-2 left-2 p-2 text-white rounded-lg ${
                      product.status.toLowerCase() === 'sales' ? 'bg-orange-500' : 'bg-green-500'
                    } hidden group-hover:block`}
                  >
                    {product.status}
                  </div>
                  <div
                    className={`absolute top-2 right-2 p-2 rounded-lg cursor-pointer ${isProductLiked ? 'text-red-500' : 'text-gray-500'}`}
              ...
              <div className="absolute bottom-0 left-0 w-full p-4 flex justify-between bg-white bg-opacity-80">
